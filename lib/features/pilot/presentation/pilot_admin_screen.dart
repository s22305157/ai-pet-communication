...
class _PilotAdminScreenState extends State<PilotAdminScreen> {
  ...
  PilotCursor? cursor;
  @override
  Widget build(BuildContext context) => PilotSurface<PilotAdminView>(
    title: '試營運管理',
    repository: widget.repository,
    loader: () async {
      final dashboard = await widget.repository.execute(
        const GetPilotDashboard(),
      );
      final page = await widget.repository.execute(
        ListPilotData(kind: kind, cursor: cursor),
      );
      return PilotAdminView(dashboard, page);
    },
    builder: (context, c) => c.value == null
        ...
        : ListView(
            ...
            children: [
              ...
              if (c.value != null) ...[
                Text(
                  '受邀資料 ${c.value!.dashboard.participants} 筆 · 待處理檢舉 ${c.value!.dashboard.openReports} 件 · 停權 ${c.value!.dashboard.suspended} 人',
                ),
                ...
                ExpansionTile(
                  ...
                  children: [
                    for (final flag in [
                      ...
                      SwitchListTile(
                        title: Text(flag.$2),
                        value: c.value!.dashboard.flags[flag.$1] == true,
                        onChanged: c.busy
                            ? null
                            : (v) async {
                                if (await pilotConfirm(
                                  ...
                                  await c.submit(SetPilotFlags({flag.$1: v}));
                                }
                              },
                      ),
                  ],
                ...
                ExpansionTile(
                  ...
                  children: [
                    for (final e
                        in c.value!.dashboard.lastSevenDayEvents.entries)
                      ListTile(
                    ...
                    if (c.value!.dashboard.truncated == true)
                      const Text('資料超過本頁彙總上限，顯示部分統計。'),
                  ],
                ),
                ...
                for (final row in c.value!.page.items)
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          if (kind == 'participants') ...[
                            ...
                            Text(
                              '${row.status == 'invited' ? '受邀' : '停用'} · 到期 ${DateFormat('yyyy/MM/dd').format(DateTime.fromMillisecondsSinceEpoch(row.expiresAtMs))}',
                            ),
                            Wrap(
                              children: [
                                TextButton(
                            ...
                            Text(
                              '${row.action} · ${DateFormat('MM/dd HH:mm').format(DateTime.fromMillisecondsSinceEpoch(row.createdAtMs))}',
                            ),
                ...
                if (c.value!.page.cursor != null)
                  TextButton(
                    onPressed: c.busy
                        ? null
                        : () {
                            cursor = c.value!.page.cursor;
                            c.load();
                          },
